...
- BM25: як ми вже наводили приклад запиту "*Yann LeCun convolutional networks*" дав статті де майже нічого не було про машине навчання, натоміть дав хороші збіги по словах. Отже робимо висновок що він корисний де треба точні збіги. Наприклад пошук автора.
- Семантичний пошук: на запит "*making computers understand human emotions from text*" дав статті про соціофізику, методи вводу тексту і тд. хоча точного збігу слів з запиту в статті немає. Отже робимо висновок що він корисний коли потрібно працювати з описовими запитами. Наприклад знайти синонім слова.

Загальне правило:
- ВМ25 для пошуку по точних збігах, наприклад імена, назви, помилки коду тощо.
- Семантичний пошук коли запит більш сформований як запитання.
- Гібрийдний пошук зажди коли є можливість, бо він об'єднює обидва методи і видає найкращі результати, нівелюючи їхні недоліки.
...
Якщо чанк має 10-15 слів, то така мала кількість слів може не містити жодного семантичного сенсу. Якщо чанк 500+ слів, він може розмивати суть бо в такі кількість слів може бути включено декілька обзаців які міститимуть різну суть. Оптимальний набір залежить від задачі, однозначно. Та в загальному оптимальна кількість десь посередині, 200-250 слів.
...
Фактично нічого не змінилось би тому що при нормалізації косинусна подібність та евклідова відстань стають математично еквівалентними. Математичне представдення:
- Формула евклідової відстані: $d^2(a,b) = ||a - b||^2$
- Квадрат різниці двох векторів: $d^2(a,b) = ||a - b||^2 = (a - b) \cdot (b - a)$
- Перемножуємо кожен член і отримуємо: $d^2(a,b) = ||a||^2 - 2 \cdot (a \cdot b) + ||b||^2$
- Нормалізуємо: $d^2(a,b) =1 - 2 \cdot (a \cdot b) + 1 = 2 - 2 \cdot (a \cdot b)$
- Формула косинусної подібності: $\text{Cosine}(a,b) = \frac{a \cdot b} {||a|| \cdot ||b||}$
- Нормалізація приводить вектори до одиниці, відповідно: $\text{Cosine}(a,b) = \frac{a \cdot b} {1 \cdot 1} = a \cdot b$
- Зводимо: $d^2(a,b) = 2 - 2 \cdot \text{Cosine}(a,b)$

Відповідно якщо відстань 0 то косинусна побідність дорівнює 1
Якщо відстань максимальна 2 то косинусна подібність доірвнює -1
...
1. Обмеження в пам'яті 2гб.
2. Дозволено тільки 1 індекс.
3. Обмеження швидкості на завантаження батчів.
4. Для задачі на 10 мільйонів статей:
- для початку використовувати не Pinecone, а наприклад Qdrant
- завантажувати дані великими батчами, асинхронно 
- фільтрувати пошук ще до математичного обчислення за метаданими